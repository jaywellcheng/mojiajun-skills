...
核心函数：
- route_by_intent()   → 根据自然语言意图找Agent（加权匹配+首位优先）
...
与 CORE-01 的关系：
    agent_cards.py 的 task_types 必须与 CORE-01 module_dispatcher.py 的
    TASK_MODULE_MAP 保持同步。用 validate_cards() 做校验。
"""
...
def route_by_intent(user_message: str) -> Optional[str]:
    ...
    算法：加权匹配 + 首位优先。
    - 关键词在列表中的位置越靠前权重越高（第1个=len，最后1个=1）
    - 多个Agent竞争时，优先第一个匹配关键词位置更靠前的
